function checksum(input) {
    ...
    //   // for (var n = 0; n <input[m].length; n++) {
    //     var max = Math.max.apply(null,input[m]);
    //     var min = Math.min.apply(null,input[m]);
    //     var diff = max - min;
    //     add+=diff;
  ...
  for (var m = 0; m < input.length; m++) {
    for (var n = 0; n < input[m].length; n++) {
      for (var p = 0; p < input[m].length; p++) {
        if (input[m][n] !== input[m][p] && input[m][n]%input[m][p]===0) {
          addQuo += input[m][n]/input[m][p];
        }
      }
    }
  }
